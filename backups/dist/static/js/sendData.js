...
function selectRowsSavedInCheckedIds() {
  ...
  [...currentIdsInputs].map((item) => {
    ...
    let matchedId = checkedIds.find((checkedId) => item.value == checkedId?.id);
...
$(document).ready(function () {
  ...
  $("body").on("click", "#create_new_project, #update_project", function () {
    ...
    else if ($("#classification").val() == "SUBLEASE_PROJECT") {
      $("#alert_message").text(
  ...
  function ccSum() {
    ...
    //   // allPrice =
    //   //   allPrice +
    //   //   parseInt($(this).text()) * parseInt($(".input__shiftsAmount").val());
    // });
    ...
    let allRows = document.querySelectorAll(".tableRow__tool");
  ...
  function allSumm() {
    ...
    let usnPercent =
      1 + Number(document.querySelector(".fixedTotalCalc__usnInp").value) / 100;
