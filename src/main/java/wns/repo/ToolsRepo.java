...
public interface ToolsRepo extends PagingAndSortingRepository<Tools,Long> {
    ...
    @Query("select t from Tools t where t.status.statusTools in ('ONLEASE','WAITING','BOOKING','REPAIR') and t.status.end >=:start and t.status.start < :end")
    List<Tools> findAllByStatusForCalendar(LocalDateTime start, LocalDateTime end);
}
